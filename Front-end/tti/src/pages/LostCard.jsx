...
const STATUS_META = {
  ...
  replaced:   { color: "#15803d", bg: "#dcfce7", label: "Replaced"   },
};
...
export default function LostCard() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await lostCardAPI.getReports();
  ...
  const handleSubmit = async () => {
    setError(null);
    if (!description.trim()) { setError("Please describe how the ID was lost."); return; }
    setSubmitting(true);
    try {
      const res = await lostCardAPI.createReport({
    ...
    <div style={{ width: "100%" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          {[
            ...
            { label: "Pending",  value: requests.filter(r => r.replacement_status === "pending").length },
          ].map(s => (
      ...
      <div style={{
        ...
        <p style={{ fontSize: 13, color: DARK, lineHeight: 1.6, margin: 0 }}>
          ...
          Your new card will be ready for collection once your request is processed.
        </p>
      </div>
      ...
      {success && (
        <div style={{ background: "#f0fdf4", border: "1px solid #bbf7d0", borderRadius: 6, padding: "12px 18px", marginBottom: 16, display: "flex", alignItems: "center", gap: 10 }}>
          <span>✅</span>
          <span style={{ fontSize: 13.5, color: "#15803d", fontWeight: 600 }}>Request submitted! Please visit the Finance office to pay the replacement fee.</span>
        </div>
      )}
      {error && (
        ...
        <div style={{ background: WHITE, border: `1px solid ${P}`, borderRadius: 8, padding: "24px 26px", marginBottom: 24, boxShadow: `0 0 0 3px ${P}15` }}>
          ...
          <div style={{ marginBottom: 20 }}>
            ...
            <textarea
              ...
              rows={4}
              placeholder="Describe how the ID was lost (e.g. misplaced during travel, stolen at market…)"
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
            <button
              onClick={() => { setShowForm(false); setError(null); setDescription(""); }}
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13.5 }}>
              <thead>
                <tr style={{ background: GREY2, borderBottom: `2px solid ${GREY3}` }}>
                  {["#", "Report Date", "Description", "Status"].map(h => (
              ...
              <tbody>
                {requests.map((r, i) => {
                  const meta = STATUS_META[r.replacement_status] || STATUS_META.pending;
                  return (
                    <tr
                      key={r.id || i}
                    ...
                    >
                      ...
                      <td style={{ padding: "13px 18px", color: DARK, maxWidth: 300 }}>
                        <span style={{ display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                          {r.description || "—"}
                        </span>
                      </td>
                      <td style={{ padding: "13px 18px" }}>
                        <span style={{ background: meta.bg, color: meta.color, fontSize: 12, fontWeight: 700, padding: "3px 10px", borderRadius: 99, whiteSpace: "nowrap" }}>
                          {meta.label}
                        </span>
                      </td>
                    </tr>
